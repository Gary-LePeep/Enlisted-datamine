...
let { buyItemAction, canBuyItem } = require("%enlist/shop/buyShopItemAction.nut")
...
let purchaseButton = @() {
  ...
  hplace = ALIGN_RIGHT
  vplace = ALIGN_BOTTOM
  children = Purchase(loc("starterPack/purchase"), @() buyItemAction(starterPack.value), {
    style = { size = [SIZE_TO_CONTENT, commonBtnHeight] }
    isEnabled = canBuyItem(starterPack.value)
  })
}
