...
let itemTypesOrder = ["medkits", "medic_medkits", "medbox", "repair_kit",
  ...
  "boltaction_noscope", "rifle_grenade_launcher", "rifle_at_grenade_launcher",
  "infantry_launcher", "launcher", "antitank_rifle", "mgun", "submgun", "carbine_pistol",
  "assault_rifle", "assault_semi", "flaregun", "mortar", "flamethrower", "sideweapon",
...
function saveRenames(renames, field, variable, hasUpgrades = false) {
  ...
  let output = file(filePath, "wt+")
  if (hasUpgrades)
    output.writestring("require common.template_utils\n\n")
  output.writestring($"let shared\n  {variable} <- {hasUpgrades ? "addUpgradesToRenameTbl(" : ""}\{\{\n")
  let armies = []
...
function dumpItemTemplates() {
  ...
  foreach (field, desc in typesDesc)
    saveRenames(renames, field, $"{RENAME_FILE_PREFIX}{capFirst(field)}", desc?.hasUpgrades ?? false)
}
