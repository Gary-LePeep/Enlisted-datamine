...
let { disableNetwork, linkSteamAccount } = require("%enlSqGlob/ui/login_state.nut")
  ...
  if (disableNetwork)
    ...
    let updatePcComp = function() {
      if (isDmmDistr && !isDMMLogin)
        ...
        let kzToken = get_arg_value_by_name("wf-token")

        setCurrentLoginUi(require(kzToken ? "%enlist/login/ui/kzLoginToken.nut" :
          "%enlist/login/ui/kzLoginBrowser.nut"))
      }
      else
