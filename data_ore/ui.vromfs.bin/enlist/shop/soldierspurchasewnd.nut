...
function mkShopItemCard(idx, shopItem, armyData, offers) {
  ...
  let lockData = Computed(@() getCantBuyData(armyData, requirements, curGrowthState.value,
    curGrowthConfig.value, curGrowthProgress.value, curGrowthTiers.value, allItemTemplates.value))

  return function() {
    let isLocked = lockData.value != null || (isFreemium && needFreemiumStatus.value)
    ...
    let hasUnseenSignal = curUnseenAvailShopGuids.value?[guid] ?? false
    ...
    let lockTxt = lockData.value?.lockTxt
    ...
    return {
      watch = [curUnseenAvailShopGuids, crateContent, needFreemiumStatus, sClassesCfg,
        perkSchemes, soldierSchemes, perksList, perksStatsCfg, allItemTemplates, lockData]
